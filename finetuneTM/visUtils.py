...
def hierarchy(folder,nvec, labs): #BASED on https://scikit-learn.org/stable/auto_examples/cluster/plot_agglomerative_dendrogram.html
    ...
    l2_norms = np.linalg.norm(nvec, axis=1)
    if sum((l2_norms == 0).astype(np.int32)>0):
        print("SOME topics are empty, this can happen if they are insignificant")
        nvec[l2_norms == 0] += 1e-6
